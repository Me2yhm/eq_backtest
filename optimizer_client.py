...
class OptimizerClient:
    def __init__(self, socket_path: str | Path, *, protocol_version: str = "shm/1",
                 schema_version: str = "1.1", timeout_ms: int = 5000,
                 request_timeout_ms: int = 6000, max_control_bytes: int = 1_048_576,
                 max_shared_bytes: int = 268_435_456) -> None:
    ...
    def envelope(self) -> dict:
        return {"protocol_version": self.protocol_version, "service_epoch": self.service_epoch,
                "session_id": self.session_id}

    def connect(self) -> None:
        if self.socket is not None:
            return
        if (self.protocol_version != "shm/1" or not sys.platform.startswith("linux")
                or sys.byteorder != "little" or not hasattr(socket, "SCM_RIGHTS")):
    ...
    def _validate_capabilities(self) -> None:
        ...
        models = {(item.get("type"), version) for item in caps.get("models", []) for version in item.get("versions", [])}
        ...
        expected = {"protocol_version": "shm/1", "control": "unix_domain_socket",
                    "backend": "memfd", "numeric_dtype": "<f8", "order": "C"}
    ...
    def validate_capacity(self, n_assets: int) -> None:
        ...
        if not isinstance(service_bytes, int) or required_bytes > min(service_bytes, self.max_shared_bytes):
            raise OptimizerClientError("optimizer shared-memory capacity is below configured portfolio size",
                                       self.capabilities)
    ...
    def optimize(self, request: dict) -> tuple[OptimizerLease, float]:
        ...
        try:
            _send_packet(self.socket, {"type": "OPTIMIZE", "request": request, **self.envelope},
        ...
        expected_response_fields = {
            "type", "protocol_version", "service_epoch", "session_id", "schema_version",
            "request_id", "status", "warnings", "errors", "solution", "diagnostics",
        }
        if (set(response) != expected_response_fields or response.get("type") != "OPTIMIZE_RESULT"
                or response.get("warnings") != [] or response.get("errors") != []):
            os.close(fds[0])
        ...
        expected = {"generation": 1, "offset": 0, "nbytes": len(asset_ids) * 8,
                    "shape": [len(asset_ids)], "dtype": "<f8", "order": "C", "role": "output"}
        if (not isinstance(descriptor, dict) or set(descriptor) != {"buffer_id", *expected}
                or not isinstance(descriptor.get("buffer_id"), str) or any(
        ...
        if (not isinstance(diagnostics, dict) or diagnostics.get("model_type") != model.get("type")
        ...
        return OptimizerLease(
            self, descriptor, mapping, weights, response, fds[0], mapping_validation_ms
        ), wait_ms
    ...
    def _consume_and_release(self, descriptor: dict) -> None:
        ...
        released = self._round_trip({"type": "RELEASE", "buffer_id": descriptor["buffer_id"],
                                     "generation": descriptor["generation"], **self.envelope})
